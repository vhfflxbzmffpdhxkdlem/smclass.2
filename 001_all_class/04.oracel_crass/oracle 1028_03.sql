...
-- number(3)
-- varchar2(20) - 문자열
-- no,name,kor,eng,math,total,acg,rank 컬럼을 구성해서 테이블을 구현하시오.
...
1,'2024-09-28',sysdata
);

select * from date_tab;

select to_char(s_date,'YYYY-MM-DD'),to_char(s_date2,yyyy-mm-dd hh24:mi:ss') form date_tab; 


select to_char(s_date,'yyyy-mm-dd hh:mi:ss'),to_char(s_date2,yyyy-mm-dd hh24:mi:ss') form date_tab;


-- 검색
-- select 컬럼명 from 테이블명
-- select * (모든컴럼의미) from 테이블명
select * from employees;

select,employee_id,emp_name,salary from employees;
SELECT EMP_ NAME FROM EMPLOYEES;
...
select employee_id,phone_number,emp_name,hire_date form employees;

-- 사원번호, 사원명, 직급번호(job_id) SH CLERK 사원만 출력화시오.
...
-- 테이블 생성(create),삭제(drp[)
-- 데이터추가(insert),삭제()delete,검색(select),수정(update)

-- delete member;

-- 삭제, where 저건절 컬럼에 행당되는 값
