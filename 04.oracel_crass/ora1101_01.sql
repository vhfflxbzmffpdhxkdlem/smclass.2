...
select sdate,add_months(sysdate,-6) from students where sdate >= add_months(sysdate,-6) order by sdate; 
...
create table emp5 as select * from employees where 1=2;

select * from emp5;

-- 테이블 구조가 있는 상태에서 모든 데이터를 입력하는 방법
insert into emp5 select * from employees;
rollback;

-- 제약조건을 확인
insert into emp5(employee_id,emp_name,hire_date) select employee_id,emp_name,hire_date from employees;
...
-- 컬럼변경 : 컬럼안에 데이터가 있다면 제약조건, 65의 길이의 문자가 있을경우, 50으로 변경불가
...
alter table emp4 modify(emp_name varchar2(17));
...
select * from emp5;
alter table emp5 modify(email number(6));
desc emp5;

alter table emp5 modify emp_name number(20);

-- employee_id 값을 email컬럼에 복사
update emp5 set email = employee_id;

-- employee_id 값을 phome_number 입력하시오
-- phone_number문자형 타입,employee_id 숫자형 타입
update emp5 set phone_number = employee_id;

commint;
rollback;

select * from emp5;
update emp5 set phone_number = '198a' where employee_id = 198;

-- 문자형타입을 숫자형 타입에 복사
-- 안에 있는 데이터가 모두 숫자형이기에 가능
-- 문자가 포함되어 있으면 타입변경이 불가
update emp5 set manager_id  = phone_number;

-- 컬럼이름 변경
desc emp5;
alter table emp5 rename column phone_number to p_number;

-- 속성 변경가능
alter table emp5 modify hire_date date null;
alter table emp5 modify hire_date date not null;

-- 컬럼 삭제
alter table emp5 drop column hire_date2;
...
insert into bmember (id,pw,name,nicname,age,gender,email,bdate) values(
'bbb','2222','유관순','관순스',20,'Female','bbb@bbb.com',sysdate
);
insert into bmember (id,pw,name,nicname,age,gender,email,bdate) values(
...
'ggg','1111','홍길자','aaa@aaa.com','123-456-7890','Female','golf',sysdate
);

commit;

-----
-- primaty key 등록시 null값이 존재하면 안됨, 중복도 존재하면 안됨.
-- primaty key 추가, 수정
