...
def breply(request,bno):
  if request.method =="GET":
    ...
    bgroup = int(request.POST.get('bgroup'))    # 답글들 그루핑
    bstep = int(request.POST.get('bstep'))      # 답글의 순서
    bindent = int(request.POST.get('bindent'))  # 들려쓰기
    btitle = request.POST.get('btitle')         # 제목
    bcontent = request.POST.get('bcontent')     # 내용
...
def bview(request,bno):
  ...
  print("cook_name : ",request.COOKIES.get('cookie_name'))
  ...
  else: # cookie_name 조재하지 않으면 - 처음으로 게시글 조회
